...
from app.models.database_models import PersonalityInstruction, HumorPhrase
import random
...
class AIService:
    ...
    def _build_system_prompt(self, db: Session) -> str:
        ...
        instructions = db.query(PersonalityInstruction).filter(
            PersonalityInstruction.is_active == True
        ).order_by(PersonalityInstruction.priority.desc()).all()
...
"""
        ...
        humor_categories = db.query(HumorPhrase.category).filter(
