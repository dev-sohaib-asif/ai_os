...
from app.models.schemas import HumorPhraseCreate, HumorPhraseResponse
from app.database.connection import get_db
from app.models.database_models import HumorPhrase
...
def add_humor_phrase(
    phrase: HumorPhraseCreate,
...
def get_humor_phrases(
    ...
    limit: int = Query(50, ge=1, le=200, description="Maximum number of phrases to return"),
    db: Session = Depends(get_db)
):
    """
...
def update_humor_phrase(
    phrase_id: int,
    phrase_update: HumorPhraseCreate,
    db: Session = Depends(get_db)
):
    """
...
def get_phrase_categories(db: Session = Depends(get_db)):
    ...
    categories = db.query(HumorPhrase.category).filter(
        HumorPhrase.is_active == True
    ).distinct().all()
...
def get_humor_stats(db: Session = Depends(get_db)):
    ...
    most_used = db.query(HumorPhrase).order_by(
        HumorPhrase.usage_count.desc()
    ).first()
    
    # Get cutest phrase
    cutest = db.query(HumorPhrase).filter(
        HumorPhrase.is_active == True
    ).order_by(HumorPhrase.cuteness_level.desc()).first()
    ...
    for cat in categories:
        count = db.query(HumorPhrase).filter(
            HumorPhrase.category == cat[0],
            HumorPhrase.is_active == True
        ).count()
