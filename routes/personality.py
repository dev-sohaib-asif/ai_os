...
from app.models.schemas import PersonalityInstructionCreate, PersonalityInstructionResponse
from app.database.connection import get_db
from app.models.database_models import PersonalityInstruction

router = APIRouter(prefix="/personality", tags=["Personality Management"])


@router.post("/instructions", response_model=PersonalityInstructionResponse, status_code=201)
def add_personality_instruction(
    instruction: PersonalityInstructionCreate,
...
def get_personality_instructions(
    instruction_type: Optional[str] = Query(None, description="Filter by instruction type"),
    ...
    if instruction_type:
        query = query.filter(PersonalityInstruction.instruction_type == instruction_type)
...
@router.get("/instructions/{instruction_id}", response_model=PersonalityInstructionResponse)
def get_personality_instruction(
    ...
    instruction = db.query(PersonalityInstruction).filter(
        PersonalityInstruction.id == instruction_id
    ).first()
...
@router.put("/instructions/{instruction_id}", response_model=PersonalityInstructionResponse)
...
def get_instruction_types(db: Session = Depends(get_db)):
    ...
    types = db.query(PersonalityInstruction.instruction_type).filter(
        PersonalityInstruction.is_active == True
    ).distinct().all()
    
    type_list = [t[0] for t in types]
    
    return {
        "instruction_types": type_list,
        "count": len(type_list)
    }
